...
function waitForRide(){const nf=window.nightfly;if(!nf?.groups?.gondola0||!nf?.state){requestAnimationFrame(waitForRide);return;}if(initialized)return;initialized=true;installUI(nf);buildRestraints(nf);requestAnimationFrame(update);}
...
function installUI(nf){
 ...
 $('spawn-guests').onclick=()=>spawnGuests(nf);
...
function buildRestraints(nf){
 ...
 for(let k=0;k<4;k++){
  const g=nf.groups['gondola'+k],t=k*Math.PI/2,radial=new THREE.Vector3(Math.cos(t),Math.sin(t),0),axle=new THREE.Vector3(-Math.sin(t),Math.cos(t),0),up=new THREE.Vector3(0,0,1);
  const gondolaBars=[];
  for(let seat=0;seat<4;seat++){
   const lateral=axle.clone().multiplyScalar((seat-1.5)*.68),pivot=lateral.clone().add(radial.clone().multiplyScalar(.03)).add(up.clone().multiplyScalar(.78));
   const holder=new THREE.Group();holder.position.copy(pivot);g.add(holder);
   const p0=axle.clone().multiplyScalar(-.22),p1=axle.clone().multiplyScalar(.22),down=radial.clone().multiplyScalar(.30).add(up.clone().multiplyScalar(-.53));
   holder.add(tube(p0,p0.clone().add(down),.035,mat),tube(p1,p1.clone().add(down),.035,mat),tube(p0.clone().add(down),p1.clone().add(down),.038,mat));
   holder.userData.axle=axle.clone();holder.userData.current=0;gondolaBars.push(holder);
  }
...
function setRestraints(close,nf){
 ...
 restraintsClosed=close;$('restraints').textContent=close?'BÜGEL ÖFFNEN':'BÜGEL SCHLIESSEN';$('restraints').classList.toggle('released',!close);setStatus(close?'Bügel geschlossen. Fahrbetrieb freigegeben.':'Bügel offen. Boarding ist möglich.');
}
...
async function toggleMic(){
 ...
 try{
  micStream=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true}});audioCtx=new AudioContext();micSource=audioCtx.createMediaStreamSource(micStream);micGain=audioCtx.createGain();const compressor=audioCtx.createDynamicsCompressor();micGain.gain.value=.85;micSource.connect(micGain).connect(compressor).connect(audioCtx.destination);button.classList.add('active');button.querySelector('b').textContent='LIVE';setStatus('Mikrofon live. Kopfhörer empfohlen, sonst kann es Rückkopplungen geben.');
 }catch(e){setStatus('Mikrofon konnte nicht aktiviert werden. Browser-Berechtigung prüfen.');}
}

function makeGuest(){
 const root=new THREE.Group(),skin=new THREE.MeshStandardMaterial({color:new THREE.Color().setHSL(.06+Math.random()*.05,.35,.58),roughness:.8}),shirt=new THREE.MeshStandardMaterial({color:new THREE.Color().setHSL(Math.random(),.55,.45),roughness:.75}),dark=new THREE.MeshStandardMaterial({color:'#24282c',roughness:.9});
 const torso=new THREE.Mesh(new THREE.CapsuleGeometry(.18,.48,5,10),shirt);torso.position.z=.72;const head=new THREE.Mesh(new THREE.SphereGeometry(.14,14,10),skin);head.position.z=1.23;const leg1=new THREE.Mesh(new THREE.CapsuleGeometry(.07,.42,4,8),dark),leg2=leg1.clone();leg1.position.set(-.09,0,.26);leg2.position.set(.09,0,.26);root.add(torso,head,leg1,leg2);root.scale.setScalar(.88+Math.random()*.16);return root;
}
...
function spawnGuests(nf){
 ...
 const occupied=new Set(guestActors.filter(a=>a.seated).map(a=>`${a.gondola}-${a.seat}`)),free=[];for(let g=0;g<4;g++)for(let seat=0;seat<4;seat++)if(!occupied.has(`${g}-${seat}`))free.push([g,seat]);
 if(!free.length){setStatus('Alle 16 Plätze sind belegt.');return;}
 const count=Math.min(free.length,Math.max(2,Math.floor(3+Math.random()*7)));
 for(let i=0;i<count;i++){const pick=free.splice(Math.floor(Math.random()*free.length),1)[0],actor=makeGuest();actor.position.set(-4.5+i*.65,-8.5-Math.random()*1.4,0);nf.scene.add(actor);guestActors.push({obj:actor,gondola:pick[0],seat:pick[1],progress:0,seated:false,delay:i*.22});}
 $('guest-count').textContent=`${guestActors.length} Gäste`;setStatus(`${count} Gäste kommen zur Anlage und steigen automatisch ein.`);
}
