...
function installUI(nf){
 ...
 const section=document.createElement('section');section.className='control-card extras-card';section.innerHTML=`
  <header class="card-title"><div><span class="card-index">05</span><h2>Operator Extras</h2></div><output id="restraint-state">16 / 16 zu</output></header>
  ...
  <p id="extras-status" class="hint">Die vorhandenen Nightfly-Schulterbügel sind geschlossen.</p>
...
function installInterlock(){
 document.addEventListener('click',e=>{
  ...
  setStatus('Fahrfreigabe gesperrt: zuerst alle Bügel schließen.');
 },true);
}

function matchesSeatRestraint(name,k,seat){
 const prefix=`Gondola_${k+1}_Seat_${seat+1}_`;
 return name.startsWith(prefix)&&(name.includes('shoulder_restraint')||name.includes('restraint_bridge')||name.includes('grab_handle'));
}
...
function buildOriginalRestraintRigs(nf){
 ...
 for(let k=0;k<4;k++){
  ...
  for(let seat=0;seat<4;seat++){
   const parts=gondola.children.filter(obj=>obj.isMesh&&matchesSeatRestraint(obj.name||'',k,seat));
   ...
   const center=box.getCenter(new THREE.Vector3()),pivot=new THREE.Vector3(center.x,center.y,box.max.z-.035);
 ...
 const state=$('restraint-state');if(state)state.textContent=`${found} / 16 zu`;
 if(found!==16)setStatus(`Bügelsteuerung aktiv. ${found} von 16 vorhandenen Bügelgruppen wurden erkannt.`);
}
...
function setRestraints(close,nf){
 ...
 $('restraints').textContent=close?'BÜGEL ÖFFNEN':'BÜGEL SCHLIESSEN';
 $('restraints').classList.toggle('released',!close);
 $('restraint-state').textContent=close?`${restraintRigs.length} / 16 zu`:`0 / 16 zu`;
 setStatus(close?'Bügel geschlossen. Fahrbetrieb freigegeben.':'Bügel geöffnet. Fahrantrieb ist verriegelt.');
}

async function toggleMic(){
 const button=$('mic-toggle');
 if(micStream){
  micStream.getTracks().forEach(t=>t.stop());micStream=null;micSource?.disconnect();micGain?.disconnect();
  if(audioCtx){await audioCtx.close();audioCtx=null;}
  button.classList.remove('active');button.querySelector('b').textContent='OFF';setStatus('Mikrofon aus.');return;
 }
 try{
  micStream=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true}});
  audioCtx=new AudioContext();micSource=audioCtx.createMediaStreamSource(micStream);micGain=audioCtx.createGain();
  const compressor=audioCtx.createDynamicsCompressor();micGain.gain.value=.85;micSource.connect(micGain).connect(compressor).connect(audioCtx.destination);
  button.classList.add('active');button.querySelector('b').textContent='LIVE';setStatus('Mikrofon live. Kopfhörer empfohlen, sonst kann es Rückkopplungen geben.');
 }catch(e){setStatus('Mikrofon konnte nicht aktiviert werden. Browser-Berechtigung prüfen.');}
}

function parseYouTube(value){
 value=value.trim();if(/^[\w-]{11}$/.test(value))return value;
 try{const u=new URL(value);if(u.hostname.includes('youtu.be'))return u.pathname.slice(1).split('/')[0];if(u.searchParams.get('v'))return u.searchParams.get('v');const m=u.pathname.match(/\/(?:embed|shorts)\/([\w-]{11})/);return m?.[1]||null;}catch{return null;}
}
function loadYouTube(){
 const id=parseYouTube($('youtube-url').value);if(!id){setStatus('Bitte gültigen YouTube-Link oder Video-ID eingeben.');return;}
 const box=$('youtube-player');box.hidden=false;box.innerHTML=`<iframe src="https://www.youtube-nocookie.com/embed/${id}?autoplay=1&controls=1&rel=0" title="Nightfly Musik" allow="autoplay; encrypted-media; picture-in-picture" allowfullscreen></iframe>`;
 setStatus('YouTube-Musik geladen. Start kann je nach Browser noch einen Klick im Player brauchen.');
}
...
function update(now){
 ...
 const dt=Math.min((now-previous)/1000,.05);previous=now,target=restraintsClosed?0:-1.08;
 for(const rig of restraintRigs){
  rig.userData.current+=(target-rig.userData.current)*Math.min(1,dt*6.5);
  rig.setRotationFromAxisAngle(rig.userData.axis,rig.userData.current);
 }
