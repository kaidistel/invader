...
function cleanOriginalGondolas(nf){
 nf.scene.traverse(o=>{
  if(!o.isMesh)return;
  const n=o.name||'';
  // These are the crude rectangular backing shells generated by build_model.py.
  // Keep the actual seat pad, back pad, headrest and restraint hardware.
  if(/Gondola_\d+_Seat_\d+_shell$/i.test(n))o.visible=false;
 });
}
