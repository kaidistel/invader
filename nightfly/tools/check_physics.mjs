...
import {RidePhysics,wrap,MAX_SIDE_ANGLE,RIDE_LIFT,MAX_LIFT,RIDE_HANG} from '../physics.mjs';
...
p.setLift(RIDE_LIFT);run(12);assert.ok(p.canDrive);assert.ok(Math.abs(p.lift-RIDE_LIFT)<.001);assert.ok(Math.abs(p.mainAngle-RIDE_HANG)<.015,'swing axis must hold the passenger arm at the angled ride pose');assert.ok(p.pivot()[2]>10);
...
p.setSwing(false);run(10);assert.ok(Math.abs(p.mainAngle-RIDE_HANG)<.015,'stopped swing must return to angled ride hold');
...
console.log('PASS: level hinges; loading interlock; fixed angled swing-axis ride pose; side-swing envelope; signed rotation; 4 independent free gondolas; collective/individual brakes; automatic parking; extreme-control stability.');
