...
import {RidePhysics,wrap,MAX_SIDE_ANGLE,RIDE_TILT} from '../physics.mjs';
...
p.setLift(1);run(10);assert.ok(p.canDrive);assert.ok(p.pivot()[2]>10);assert.ok(Math.abs(p.mainAngle-RIDE_TILT)<.01,'ride pose should remain slightly angled');
...
p.reset();p.setLift(1);run(10);p.setSwing(true);p.setSpin(true);p.amplitude=130*Math.PI/180;p.swingSpeed=40*Math.PI/180;p.spinRPM=-12;p.setBrakes(false);run(60);assert.ok(Math.abs(p.mainAngle)<=MAX_SIDE_ANGLE+.001);assert.ok(p.gondolas.every(g=>Math.abs(g.velocity)<=18));p.setLift(0);run(70);assert.ok(p.lift<.001&&!p.parking);
console.log('PASS: level hinges; loading interlock; angled ride pose; side-swing envelope; signed rotation; 4 independent free gondolas; collective/individual brakes; automatic parking; extreme-control stability.');
...
p.reset();p.setLift(1);run(10);p.setSwing(true);p.setSpin(true);p.setBrakes(false);run(5);
