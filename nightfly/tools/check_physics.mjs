...
import {RidePhysics,wrap,MAX_SIDE_ANGLE,RIDE_LIFT,MAX_LIFT,rx,ry,rz,LIFT_ANGLE} from '../physics.mjs';
...
p.setLift(RIDE_LIFT);run(12);assert.ok(p.canDrive);assert.ok(Math.abs(p.lift-RIDE_LIFT)<.001);assert.ok(Math.abs(p.mainAngle)<.01,'swing motor must be neutral when off');assert.ok(p.pivot()[2]>10);
const baseArm=model.origins.crown.map((v,i)=>v-model.origins.rotor[i]);const inherited=rx(baseArm,LIFT_ANGLE*p.lift);assert.ok(Math.abs(inherited[1]-baseArm[1])>.1||Math.abs(inherited[2]-baseArm[2])>.1,'ride pose must inherit lift-boom inclination');
...
p.setSwing(false);run(10);assert.ok(Math.abs(p.mainAngle)<.015,'stopped swing must return to local neutral while retaining inherited boom tilt');
...
console.log('PASS: level hinges; loading interlock; lift-inherited swing-axis inclination; side-swing envelope; signed rotation; 4 independent free gondolas; brakes; parking; stability.');
...
p.reset();p.setLift(RIDE_LIFT);run(12);p.setSwing(true);p.setSpin(true);p.setBrakes(false);run(5);
const liftAngle=LIFT_ANGLE*p.lift;lift.rotation.x=liftAngle;rotor.rotation.y=p.mainAngle;crown.rotation.z=p.spinAngle;lift.updateMatrixWorld(true);
for(let k=0;k<4;k++){const actual=new THREE.Vector3(...model.origins['gondola'+k]).sub(new THREE.Vector3(...model.origins.crown)).applyMatrix4(crown.matrixWorld);assert.ok(actual.distanceTo(new THREE.Vector3(...p.support(k)))<1e-9);}
console.log('PASS: Three.js transforms match physics with the swing axle rigidly inheriting the lift boom rotation.');
