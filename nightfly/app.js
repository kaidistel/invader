...
function syncOnrideCamera(){
 if(cameraPreset!=='onride'||!onrideSeat)return;
 const {gondola,seat}=onrideSeat,g=groups['gondola'+gondola],t=gondola*Math.PI/2;
 const radial=new THREE.Vector3(Math.cos(t),Math.sin(t),0),axle=new THREE.Vector3(-Math.sin(t),Math.cos(t),0),up=new THREE.Vector3(0,0,1);
 // Eye point is slightly forward of the backrest and below the top of the headrest.
 const localEye=axle.clone().multiplyScalar((seat-1.5)*.68).add(radial.clone().multiplyScalar(.18)).add(up.clone().multiplyScalar(.48));
 const localTarget=localEye.clone().add(radial.clone().multiplyScalar(4)).add(up.clone().multiplyScalar(.06));
 const worldEye=g.localToWorld(localEye.clone()),worldTarget=g.localToWorld(localTarget.clone());
 const worldQ=g.getWorldQuaternion(new THREE.Quaternion()),worldUp=up.clone().applyQuaternion(worldQ).normalize();
 camera.position.copy(worldEye);camera.up.copy(worldUp);camera.lookAt(worldTarget);
}
function syncPose(){
 const liftAngle=LIFT_ANGLE*engine.lift;groups.lift.rotation.x=liftAngle;
 groups.rotor.rotation.set(0,engine.mainAngle,0);groups.crown.rotation.z=engine.spinAngle;
...
function syncUI(){
 $('lift-out').textContent=`${Math.round(engine.lift*100)} %`;$('swing-out').textContent=`${Math.round(deg(wrap(engine.mainAngle)))}°`;$('spin-out').textContent=`${(engine.spinVelocity*60/(2*Math.PI)).toFixed(1)} U/min`;
 ...
 $('swing').disabled=$('spin').disabled=!engine.canDrive;$('brake').disabled=engine.parking;$('swing').classList.toggle('active',engine.swingOn);$('spin').classList.toggle('active',engine.spinOn);
 ...
 $('mode').textContent=paused?'Simulation pausiert':cameraPreset==='onride'&&onrideSeat?`Onride · Gondel ${onrideSeat.gondola+1}, Sitz ${onrideSeat.seat+1}`:engine.parking?'Richtet sich zur Beladung aus':engine.lift<.01?'Ladeposition':!engine.canDrive?'Hubarm bewegt sich':engine.swingOn||engine.spinOn?'Fahrt läuft':'Fahrposition';
...
function setCamera(preset){
 cameraPreset=preset;document.querySelectorAll('[data-camera]').forEach(b=>b.classList.toggle('selected',b.dataset.camera===preset));orbit.enabled=preset!=='onride';camera.up.set(0,0,1);
 if(preset==='onride'){
  onrideSeat={gondola:Math.floor(Math.random()*4),seat:Math.floor(Math.random()*4)};syncPose();syncUI();return;
 }
 onrideSeat=null;const center=groups.crown?groups.crown.getWorldPosition(new THREE.Vector3()):new THREE.Vector3(0,-2.8,2.4);
...
try{
 ...
 // Do not add the old runtime axle/seat overlay here. The JSON model already
 // contains the actual carrier, seats, restraints and guards with head clearance.
 makeHydraulics();syncPose();syncUI();ready=true;$('loading').hidden=true;
...
$('reset').onclick=()=>{if(!ready)return;engine.reset();paused=false;$('pause').textContent='Pause';$('lift').value=0;$('amplitude').value=78;$('amplitude-out').value='78°';$('swing-speed').value=20;$('swing-speed-out').value='20°/s';$('spin-speed').value=6;$('spin-speed-out').value='6 U/min';setCamera('overview');syncPose();syncUI();};
