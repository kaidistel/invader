...
import {RidePhysics,LIFT_ANGLE,wrap,clamp} from './physics.mjs';
...
function syncUI(){
 ...
 $('load').classList.toggle('selected',engine.liftTarget===0||engine.parking);$('ride').classList.toggle('selected',engine.liftTarget===1&&!engine.parking);
 ...
 $('mode').textContent=paused?'Simulation pausiert':engine.parking?'Richtet sich zur Beladung aus':engine.lift<.01?'Ladeposition':engine.lift<.94?'Hubarm bewegt sich':engine.swingOn||engine.spinOn?'Fahrt läuft':'Fahrposition';
 $('lift-hint').textContent=engine.parking?'Antriebe stoppen, Gondeln ausrichten, dann absenken.':engine.canDrive?'Fahrposition erreicht. Die Antriebe sind freigegeben.':'Zum Starten zuerst in Fahrposition anheben.';
}
function setCamera(preset){cameraPreset=preset;document.querySelectorAll('[data-camera]').forEach(b=>b.classList.toggle('selected',b.dataset.camera===preset));const center=groups.crown?groups.crown.getWorldPosition(new THREE.Vector3()):new THREE.Vector3(0,-2.8,2.4);
 if(preset==='front'){camera.position.set(0,-32,10);orbit.target.set(0,-1,6);}else if(preset==='seats'){camera.position.copy(center).add(new THREE.Vector3(7,-10,6));orbit.target.copy(center);}else{camera.position.set(17,-29,16);orbit.target.set(0,-1.5,5.5);}orbit.update();}
try{
