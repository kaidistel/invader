...
export class RidePhysics{
 ...
 reset(){this.lift=0;this.liftVelocity=0;this.liftTarget=0;this.mainAngle=0;this.mainVelocity=0;this.mainAccel=0;this.spinAngle=0;this.spinVelocity=0;this.spinAccel=0;this.phase=0;this.swingOn=false;this.spinOn=false;this.amplitude=85*Math.PI/180;this.swingSpeed=20*Math.PI/180;this.spinRPM=6;this.parking=false;this.afterPark=0;this.time=0;this.gondolas=Array.from({length:4},()=>({angle:0,velocity:0,brake:true,previous:null,prevVelocity:[0,0,0],acc:[0,0,0]}));}
 ...
 support(k){const local=sub(this.origins['gondola'+k],this.origins.crown);return add(this.pivot(),rx(add(sub(this.origins.crown,this.origins.rotor),rz(local,this.spinAngle)),this.mainAngle));}
 step(dt){
  this.time+=dt;
  ...
  if(this.swingOn)this.phase+=dt*this.swingSpeed/Math.max(.1,this.amplitude);
  const target=this.swingOn?this.amplitude*Math.sin(this.phase):0;
  ...
  const axisSpin=rx([0,0,this.spinVelocity],this.mainAngle);const omega=add([this.mainVelocity,0,0],axisSpin);const alpha=add(add([this.mainAccel,0,0],rx([0,0,this.spinAccel],this.mainAngle)),cross([this.mainVelocity,0,0],axisSpin));
  for(let k=0;k<4;k++){
   ...
   const t=k*Math.PI/2,hinge=rx(rz([-Math.sin(t),Math.cos(t),0],this.spinAngle),this.mainAngle);
   const down=rx([0,0,-1],this.mainAngle),side=cross(hinge,down),length=.62;
